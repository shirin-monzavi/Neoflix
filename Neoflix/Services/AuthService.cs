    ...
    public class AuthService
    {
        ...
        public async Task<Dictionary<string, object>> RegisterAsync(string email, string plainPassword, string name)
        {
            try
            {
                ...
                //{
                //    throw new ValidationException(@"An account already exists with the email", email);
                //}
                // TODO: Save user
                ...
                {
                    var query = @"Merge (u:User {userId:randomUuid(),
                                                email:$email,
                                                password:$encrypted,
                                                name:$name}) return u {.userId , .name,.email} as u";

                    var cursor = await tx.RunAsync(query, new { email, encrypted, name });
                    var record = await cursor.SingleAsync();
                    var result = record["u"].As<Dictionary<string, object>>();

                    var safeProperties = SafeProperties(result);
        ...
        public async Task<Dictionary<string, object>> AuthenticateAsync(string email, string plainPassword)
        {
            ...
            return await session.ExecuteReadAsync(async (tx) =>
             {
                 var cursor = await tx.RunAsync("match (u:User) where u.email= $email return u", new { email });
                 ...
                 if (await cursor.FetchAsync())
                 {
                     ...
                     var user = userProperties.ToDictionary(x => x.Key, x => x.Value);

                     if (BCryptNet.Verify(plainPassword, user["password"].As<string>()))
                     {
                         var safeProperties = SafeProperties(userProperties as Dictionary<string, object>);
